...
import { list } from 'postcss';
    ...
    const Add = () => {
        ...
        user.wallets.list[currIndex].transactions = [...user.wallets.list[currIndex].transactions, transaction]
...
<input id="addAmount" type="number" placeholder="add" bind:value={add} class="w-64 mx-auto bg-slate-300 p-2 rounded-lg m-2">
<button id="addButton" on:click={Add} class=" bg-blue-400 w-32 p-2 mx-auto m-2 rounded transition-all hover:bg-blue-200 active:bg-white">Add</button>

<input id="newWalletName" type="text" placeholder="wallet name" bind:value={newName} class="w-64 mx-auto bg-slate-300 p-2 rounded-lg m-2">
<button id="newWalletButton" on:click={newWallet} class=" bg-blue-400 w-32 p-2 mx-auto m-2 rounded transition-all hover:bg-blue-200 active:bg-white">Add</button>
